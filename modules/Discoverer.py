...
class Discoverer(pl.LightningModule):
    ...
    def validation_step(self, data,_):
        ...
        #coords, feats, real_labels, _, _, t = data  # 获取 t 作为 frame_id
        #
        # # 打印数据尺寸信息
        ...
        # # 保存当前批次的特征和标签---tsne
        # features = out["feats"].detach().cpu().numpy()  # 将特征从 GPU 转到 CPU
        # self.features_list.append(features)
        # self.labels_list.append(real_labels.cpu().numpy())  # 保存真实标签
        ...
        #
        # # 调试代码，分析预测类别的数量分布
        # unique_labels, counts = np.unique(pred_labels.detach().cpu().numpy(), return_counts=True)
        # print(f"Unique predicted labels in frame: {unique_labels}")
        ...
        #     # 获取当前样本的 frame_id
        #     frame_id_tensor = t[i]
        #     frame_id = frame_id_tensor.item()  # 转为 Python 标量
        #     # 找到属于当前批次 i 的所有点
        #     batch_indices = coords[:, 0] == i  # 通过 batch_id 找到属于当前样本的点
        #     coords_sample = coords[batch_indices, 1:]  # 提取属于该批次的所有点并去掉 batch_id，形状为 (N, 3)
        #     pred_labels_sample = pred_labels[batch_indices]  # 提取属于该批次的所有预测标签
        #     # 打印调试信息
        #     print(f"Processing sample {i}, frame_id: {frame_id}")
        #     print(f"coords_sample.shape: {coords_sample.shape}")
        #     print(f"pred_labels_sample.shape: {pred_labels_sample.shape}")
        #     assert coords_sample.shape[0] == pred_labels_sample.shape[
        #         0], f"Mismatch between coordinates and labels for frame {frame_id}"
        #     # 保存预测结果为 SemanticKITTI 格式
        #     save_semantickitti_format(
        #         coords=coords_sample,
        #         pred_labels=pred_labels_sample,
        #         sequence_id=8,  # 使用实际的序列编号
        #         frame_id=frame_id,  # 使用实际的帧编号
        #         output_dir='/home/zzh/ECCV-LY/NCD_PC-main/prediction_save/KITTI'
        #     )

        # if len(self.features_list) == 0 or len(self.labels_list) == 0:
        #     print("No data collected for t-SNE visualization.")
        #     return

        # # 在每个验证周期结束时执行 t-SNE 可视化
        # all_features = np.concatenate(self.features_list, axis=0)
        # all_labels = np.concatenate(self.labels_list, axis=0)
        #
        # if all_features.size == 0 or all_labels.size == 0:
        #     print("Collected data is empty. Skipping t-SNE visualization.")
        #     return
        #
        # # 使用 t-SNE 降维
        # tsne = TSNE(n_components=2, perplexity=30.0, n_iter=1000)
